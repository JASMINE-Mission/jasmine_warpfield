...
def generate(src, env, ref, params={}):
    ...
    def model():
        src_a = init.value('src_a_loc')
        src_d = init.value('src_d_loc')

        tel_a = init.value('tel_a_loc')
        tel_d = init.value('tel_d_loc')
        tel_t = init.value('tel_t_loc')
        tel_s = init.value('tel_s_loc')

        opt_A = init.value('opt_A_loc')
        opt_B = init.value('opt_B_loc')
        ...
        det_r = init.value('det_r_loc')
        det_o = init.value('det_o_loc')
        det_p = init.value('det_p_loc')
        ...
        sig_x = numpyro.sample('sig_y', prior.sig_x_dist)
        sig_y = numpyro.sample('sig_x', prior.sig_y_dist)
    ...
    def guide():
        init.param('tel_a_loc')
        init.param('tel_d_loc')
        init.param('tel_t_loc')
        init.param('tel_s_loc', constraint=c.positive)
        init.param('opt_A_loc')
        init.param('opt_B_loc')

        init.param('det_r_loc')
        init.param('det_o_loc')
        init.param('det_p_loc', constraint=c.positive)

        sig_x = init.param('sig_x_loc', constraint=c.positive)
        sig_y = init.param('sig_y_loc', constraint=c.positive)
        numpyro.sample('sig_x', dist.Delta(sig_x))
        numpyro.sample('sig_y', dist.Delta(sig_y))

        init.param('src_a_loc')
        init.param('src_a_sig', constraint=c.positive)
        init.param('src_d_loc')
        init.param('src_d_sig', constraint=c.positive)

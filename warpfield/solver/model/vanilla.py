...
def generate(source, reference, params={}):
    ...
    pri_p_sig = jnp.array(reference['sig'])
    ...
    def model():
        with numpyro.plate('ref', M):
            ra  = numpyro.sample('ra',  dist.Normal(pri_a_loc, pri_p_sig))
            dec = numpyro.sample('dec', dist.Normal(pri_d_loc, pri_p_sig))
        ...
        xy = numpyro.deterministic('xy',
              projection(ax, dx, tx, rax, dex, fx))
    ...
    def guide():
        loc_a = numpyro.param(
            'tel_a_loc', tel_a_loc,
            constraint=c.interval(tel_a_loc-1,tel_a_loc+1))
        sig_a = numpyro.param(
            'tel_a_sig', tel_a_sig, constraint=c.positive)
        loc_d = numpyro.param(
            'tel_d_loc', tel_d_loc,
            constraint=c.interval(tel_d_loc-1,tel_d_loc+1))
        sig_d = numpyro.param(
            'tel_d_sig', tel_d_sig, constraint=c.positive)
        loc_t = numpyro.param(
            'tel_t_loc', tel_t_loc,
            constraint=c.interval(tel_t_loc-5,tel_t_loc+5))
        sig_t = numpyro.param(
            'tel_t_sig', tel_t_sig, constraint=c.positive)
        loc_f = numpyro.param(
            'foc_f_loc', foc_f_loc, constraint=c.interval(0.9,1.1))
        sig_f = numpyro.param(
            'foc_f_sig', foc_f_sig, constraint=c.positive)

        with numpyro.plate('pointing', N):
            tel_a = numpyro.sample('tel_a', dist.Normal(loc_a, sig_a))
            tel_d = numpyro.sample('tel_d', dist.Normal(loc_d, sig_d))
            tel_t = numpyro.sample('tel_t', dist.Normal(loc_t, sig_t))
        foc_f = numpyro.sample('foc_f', dist.Normal(loc_f, sig_f))

        with numpyro.plate('ref', M):
            loc_ra  = numpyro.param(
                'ra_loc', ref_a_loc,
                constraint=c.interval(
                    ref_a_loc-10*ref_a_sig, ref_a_loc+10*ref_a_sig))
            sig_ra  = numpyro.param(
                'ra_sig', ref_a_sig, constraint=c.positive)
            loc_dec = numpyro.param(
                'dec_loc', ref_d_loc,
                constraint=c.interval(
                    ref_d_loc-10*ref_d_sig, ref_d_loc+10*ref_d_sig))
            sig_dec = numpyro.param(
                'dec_sig', ref_d_sig, constraint=c.positive)

            ra = numpyro.sample('ra', dist.Normal(loc_ra, sig_ra))
            dec = numpyro.sample('dec', dist.Normal(loc_dec, sig_dec))

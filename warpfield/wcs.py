...
def _generate_one_wcs(pointing, scale, detector, projection_code):
    ...
    optical_center = (rotation @ (-offset)) / pixel_scale + np.asarray(
        detector.shape
    ) / 2
    focal_to_plane = _rotation_matrix(
        float(pointing.position_angle[0])
    ) @ np.diag(1 / scale)
    detector_to_focal = _rotation_matrix(-float(detector.rotation)) @ np.diag(
        pixel_scale
    )
    ...
    wcs.wcs.cd = np.diag([-1.0, 1.0]) @ focal_to_plane @ detector_to_focal
    return wcs
...
def generate_wcs(exposure, telescope):
    ...
    scale_factors = np.asarray(exposure.calibration.scale_factor(indices))
    scales = np.asarray(telescope.optics.plate_scale)[None, :] * scale_factors

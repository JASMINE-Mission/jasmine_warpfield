...
class Telescope(zdx.Base):
    ...
    def _validate_sky(tel_ra, tel_dec, tel_pa, ra, dec):
        ...
        if tel_pa.shape != ra.shape:
            raise ValueError('`tel_pa` should have the same shape as `ra`.')

        return tel_ra, tel_dec, tel_pa, ra, dec

    def focal_plane(self, tel_ra, tel_dec, tel_pa, ra, dec, scale_factor):
        ...
        return self.optics(tel_ra, tel_dec, tel_pa, ra, dec, scale_factor)

    def __call__(
        self, tel_ra, tel_dec, tel_pa, ra, dec, scale_factor, detector_index
    ):
        """Map sky coordinates onto detector coordinates"""
        xy = self.focal_plane(tel_ra, tel_dec, tel_pa, ra, dec, scale_factor)

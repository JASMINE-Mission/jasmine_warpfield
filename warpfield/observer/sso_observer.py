...
class SSOObserver(GeoCentric):
    ...
    def orbital_period(self):
        ...
        return 2 * np.pi * np.sqrt(
            self.orbital_radius**3 / c.GM_earth)
    ...
    def inclination(self):
        ...
        solar_rate = (
            2 * np.pi * u.rad / _TROPICAL_YEAR
        ).to(1 / u.s, equivalencies=u.dimensionless_angles())
        cosine = (
            -2 * solar_rate
            / (
                3 * _EARTH_J2 * mean_motion
                * (c.R_earth / radius)**2
            )
        ).decompose().value
    ...
    def obsgeoloc(self):
        ...
        direction = (
            np.cos(argument) * ascending
            + np.sin(argument) * transverse
        )
    ...
    def obsgeovel(self):
        ...
        direction = (
            -np.sin(argument) * ascending
            + np.cos(argument) * transverse
        )

...
class Exposure(zdx.Base):
    ...
    def __init__(self, pointing, calibration=None):
        ...
        if not isinstance(calibration, Calibration):
            raise TypeError('`calibration` should be a Calibration instance.')
    ...
    def from_qtable(cls, table):
        ...
        if kind is None:
            kind = (
                'scale'
                if 'scale_coefficient' in table.colnames
                else 'identity'
            )
        ...
        if kind == 'identity':
            ...
            coefficient = u.Quantity(table['scale_coefficient']).to_value(
                u.dimensionless_unscaled
            )
